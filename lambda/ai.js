...
async function generatePageSummary(page, openai) {
      ...
      messages: [
        {
          role: 'system',
          content:
            '당신은 비즈니스 분석 전문가로, 투자자와 파트너에게 유용한 인사이트를 제공합니다. 각 페이지 타입의 특성에 맞는 차별화된 분석을 제공하세요.',
        },
        {
          role: 'user',
          content: prompt,
...
function generatePrompt(detailLevel, content) {
...
function parseAISummary(rawResponse, detailLevel) {
  try {
...
async function generateAISummary(pages, openai, detailLevel = 'basic') {
